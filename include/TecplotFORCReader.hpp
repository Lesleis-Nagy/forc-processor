            ...
            if (std::regex_match(line, match_zone, _regex_zone)) {
                ...
                if (zone_counter == 1) {
                    ...
                    if ((!forc_curves._n_verts.has_value()) && (!forc_curves._n_elems.has_value())) {
                        throw std::runtime_error("Parsing subsequent zone, but no. of vertices/elements is not set.");
                    } else {
                    ...
                    for (const auto& str_value : str_values) {
                        if (!str_value.empty()) {
                            if (!forc_curves.x_is_full()) {
                                ...
                                forc_curves._mx[forc_curves._current_field_idx.value()].push_back(std::stod(str_value));
                            } else if (!forc_curves.current_my_is_full()) {
                                forc_curves._my[forc_curves._current_field_idx.value()].push_back(std::stod(str_value));
                            } else if (!forc_curves.current_mz_is_full()) {
                                forc_curves._mz[forc_curves._current_field_idx.value()].push_back(std::stod(str_value));
                            } else {
